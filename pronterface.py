    ...
    def do_settemp(self,l="", extruder=0):
        try:
            ...
            if (self.active_extruder==0):
                ...
                setoffbuttonpointer = self.settoff
                setbuttonpointer = self.settbtn
            else:
                temppointer = self.htemp2
                setoffbuttonpointer = self.settoff2
                setbuttonpointer = self.settbtn2
            
            if not (l.__class__=="".__class__ or l.__class__==u"".__class__) or (not len(l)):
                l=str(temppointer.GetValue().split()[0])
            ...
            if f>=0:
                if self.p.online:
                    ...
                    if f>0:
                        wx.CallAfter(temppointer.SetValue,l)
                        self.set("last_temperature",str(f))
                        wx.CallAfter(setoffbuttonpointer.SetBackgroundColour,"")
                        wx.CallAfter(setoffbuttonpointer.SetForegroundColour,"")
                        wx.CallAfter(setbuttonpointer.SetBackgroundColour,"#FFAA66")
                        wx.CallAfter(setbuttonpointer.SetForegroundColour,"#660000")
                        wx.CallAfter(temppointer.SetBackgroundColour,"#FFDABB")
                    else:
                        wx.CallAfter(setoffbuttonpointer.SetBackgroundColour,"#0044CC")
                        wx.CallAfter(setoffbuttonpointer.SetForegroundColour,"white")
                        wx.CallAfter(setbuttonpointer.SetBackgroundColour,"")
                        wx.CallAfter(setbuttonpointer.SetForegroundColour,"")
                        wx.CallAfter(temppointer.SetBackgroundColour,"white")
                        wx.CallAfter(temppointer.Refresh)
                else:
                    print _("Printer is not online.")
            else:
                print _("You cannot set negative temperatures. To turn the hotend off entirely, set its temperature to 0.")
...
#        lls.Add(wx.StaticText(self.panel,-1,_("Heater 1:")),pos=(13,0),span=(1,1),flag=wx.ALIGN_CENTER_VERTICAL|wx.ALIGN_RIGHT)
        ...
        self.htemp=wx.ComboBox(self.panel, -1,
                choices=htemp_choices,style=wx.CB_DROPDOWN, size=(80,-1))
        self.htemp.Bind(wx.EVT_COMBOBOX,self.htemp_change1)

        lls.Add(self.htemp,pos=(4,5),span=(1,2))
        self.settbtn=wx.Button(self.panel,-1,_("Set"),size=(38,-1))
        self.settbtn.Bind(wx.EVT_BUTTON,lambda e:self.do_settemp("", 0))
        self.printerControls.append(self.settbtn)
        lls.Add(self.settbtn,pos=(4,7),span=(1,1))
...
#lls.Add(wx.StaticText(self.panel,-1,_("Heater 2:")),pos=(18,0),span=(1,1),flag=wx.ALIGN_CENTER_VERTICAL|wx.ALIGN_RIGHT)
        ...
        self.settoff2=wx.Button(self.panel,-1,_("Off"),size=(36,-1))
        self.settoff2.Bind(wx.EVT_BUTTON,lambda e:self.do_settemp("off", 1))
        self.printerControls.append(self.settoff2)
        lls.Add(self.settoff2,pos=(4,8),span=(1,1))
                
        if self.settings.last_temperature not in map(float,self.temps.values()):
            htemp_choices = [str(self.settings.last_temperature)] + htemp_choices
        self.htemp2=wx.ComboBox(self.panel, -1,
            choices=htemp_choices,style=wx.CB_DROPDOWN, size=(80,-1))
        self.htemp2.Bind(wx.EVT_COMBOBOX,self.htemp_change2)
                
        lls.Add(self.htemp2,pos=(4,9),span=(1,2))
        self.settbtn2=wx.Button(self.panel,-1,_("Set"),size=(38,-1))
        self.settbtn2.Bind(wx.EVT_BUTTON,lambda e:self.do_settemp("", 1))
        self.printerControls.append(self.settbtn2)
        lls.Add(self.settbtn2,pos=(4,11),span=(1,1))
...
#lls.Add(wx.StaticText(self.panel,-1,_("Bed:")),pos=(4,0),span=(1,1),flag=wx.ALIGN_CENTER_VERTICAL|wx.ALIGN_RIGHT)
        ...
        self.edist=wx.SpinCtrl(self.panel,-1,"20",min=0,max=1000,size=(60,-1))
        self.edist.SetBackgroundColour((225,200,200))
        self.edist.SetForegroundColour("black")
        lls.Add(self.edist,pos=(5,6),span=(1,1))
        lls.Add(wx.StaticText(self.panel,-1,_("mm")),pos=(5,7),span=(1,1))
...
#def scroll_setpoint(e):
    ...
    def htemp_change1(self,event):
        if self.hsetpoint1 > 0:
            self.do_settemp("", 0)
        wx.CallAfter(self.htemp.SetInsertionPoint,0)

    def htemp_change2(self,event):
        if self.hsetpoint2 > 0:
            self.do_settemp("", 1)
        wx.CallAfter(self.htemp2.SetInsertionPoint,0)
...
if __name__ == '__main__':
    ...
    try:
        ...
        pass
>>>>>>> GUI optimization, temp working, extrude still not working
